...
2. **Club.Channel.Api** (بک‌اند صف‌بندی درخواست‌های کانال)
3. **Club.EventHandler.Api** (پردازش‌گر رویداد و مصرف‌کننده صف)

همچنین نقش `Club.Channel.Contracts` و زیرساخت Neo در مدیریت صف، Outbox و Retry بیان می‌شود.
...
sequenceDiagram
    ...
    participant ChannelApi as Club.Channel.Api
    participant Queue as Neo Queue/Outbox
    participant EventHandler as Club.EventHandler.Api
...
- استفاده از endpointهای Club.Channel.Api برای ایجاد رویداد، مصرف پاداش و سایر عملیات.
- ذخیره و بازپخش سناریوها برای تست خودکار.

### Club.Channel.Api
- لایه وب برای دریافت درخواست‌های REST از چنل‌ها / شبیه‌ساز.
- تبدیل ورودی به Command/Event استاندارد با استفاده از DTOهای `Club.Channel.Contracts`.
...
### Club.EventHandler.Api
- سرویس مصرف‌کننده صف برای پردازش Command/Event ثبت شده توسط Channel.Api.
- استفاده از `Club.Channel.Contracts` برای دی‌سریال پیام.
...
2. تعیین قراردادهای اولیه در `Club.Channel.Contracts`.
