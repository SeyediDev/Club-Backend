...
# این اسکریپت connection string را از appsettings پنل ادمین Club می‌خواند و اسکریپت SQL را اجرا می‌کند

param(
    [string]$ScriptPath = "SeedPointsDistributionData.sql"
)

# مسیر فایل‌های appsettings پنل ادمین Club
$adminPanelPath = Join-Path $PSScriptRoot "..\..\..\..\AdminPanel\Club.AdminPanel.Web"
...
# اول appsettings.json اصلی را چک می‌کنیم (برای پروژه Club)
...
# فرمت 1: Server=localhost,1433;Database=ClubBpmsDev;User Id=sa;Password=...
if ($connectionString -match "Server=([^;]+)") {
    $server = $matches[1]
}
# فرمت 2: Data Source=.;Initial Catalog=Club;User ID=sa;Password=...
...
try {
    ...
    Write-Host "  1. XP Club: ~10,000,000 (largest)" -ForegroundColor White
