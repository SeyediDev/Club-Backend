...
1. **باز کردن SSMS** و اتصال به دیتابیس `ClubBpmsDev`
...
```powershell
cd D:\Projects\Club\Backend\src\Core\Club.Infrastructure\Data\Scripts
...
```cmd
sqlcmd -S localhost,1433 -d ClubBpmsDev -U sa -P "YourPassword" -i SeedPointsDistributionData.sql
```
