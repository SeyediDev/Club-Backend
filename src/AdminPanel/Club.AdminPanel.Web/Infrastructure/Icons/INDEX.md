...
```
Club.Bpms/
├── Club.Bpms/Infrastructure/Icons/
...
Icon name ها **مستقیماً** در `Menu_Club.cs` تعریف می‌شوند.

### 2. دریافت Icon از Registry

```csharp
using Club.Bpms.Infrastructure.Icons;
...
menuItem.WithClubIcon("Customer")           // Set icon
        .WithIconColor("Customer");         // Set color

// یا به صورت ترکیبی:
menuItem.WithClubIconAndColor("Customer");
```
...
3. **افزودن به UseClubIcon در MenuHelper**:
   ```csharp
   // MenuHelper.cs
