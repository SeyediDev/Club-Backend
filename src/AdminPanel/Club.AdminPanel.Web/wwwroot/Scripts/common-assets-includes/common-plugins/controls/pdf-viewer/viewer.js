...
_pdfjsLib.PDFJS.maxCanvasClubels = _pdfjsLib.PDFJS.maxCanvasClubels === undefined ? 16777216 : _pdfjsLib.PDFJS.maxCanvasClubels;
...
function getOutputScale(ctx) {
  var deviceClubelRatio = window.deviceClubelRatio || 1;
  var backingStoreRatio = ctx.webkitBackingStoreClubelRatio || ctx.mozBackingStoreClubelRatio || ctx.msBackingStoreClubelRatio || ctx.oBackingStoreClubelRatio || ctx.backingStoreClubelRatio || 1;
  var ClubelRatio = deviceClubelRatio / backingStoreRatio;
  return {
    sx: ClubelRatio,
    sy: ClubelRatio,
    scaled: ClubelRatio !== 1
  };
}
...
function normalizeWheelEventDelta(evt) {
  ...
  var MOUSE_DOM_DELTA_ClubEL_MODE = 0;
  var MOUSE_DOM_DELTA_LINE_MODE = 1;
  var MOUSE_ClubELS_PER_LINE = 30;
  var MOUSE_LINES_PER_PAGE = 30;
  if (evt.deltaMode === MOUSE_DOM_DELTA_ClubEL_MODE) {
    delta /= MOUSE_ClubELS_PER_LINE * MOUSE_LINES_PER_PAGE;
...
var PDFPageView = function () {
  ...
  _createClass(PDFPageView, [{
    ...
    value: function update(scale, rotation) {
      ...
      if (this.canvas && _pdfjsLib.PDFJS.maxCanvasClubels > 0) {
        var outputScale = this.outputScale;
        if ((Math.floor(this.viewport.width) * outputScale.sx | 0) * (Math.floor(this.viewport.height) * outputScale.sy | 0) > _pdfjsLib.PDFJS.maxCanvasClubels) {
    ...
    value: function paintOnCanvas(canvasWrapper) {
      ...
      if (_pdfjsLib.PDFJS.maxCanvasClubels > 0) {
        var ClubelsInViewport = viewport.width * viewport.height;
        var maxScale = Math.sqrt(_pdfjsLib.PDFJS.maxCanvasClubels / ClubelsInViewport);
