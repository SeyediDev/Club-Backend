...
) : IRequestHandler<EnqueueEventCommand, OutboxResponse>
{
    public async Task<OutboxResponse> Handle(EnqueueEventCommand request, CancellationToken cancellationToken)
    {
        ...
        var tags = GetTags("Club.Channel", "EnqueueEvent", request);
        using var activity = telementry.StartActivity(request, "Club.Channel", "EnqueueEvent", tags);
        var channelDto = await channelService.GetChannelByClientIdAsync(request.ClientId, cancellationToken);
        if (channelDto == null)
        {
            telementry.OnFinalize<EnqueueEventCommand, OutboxResponse>(request, null, "Club.Channel", "EnqueueEvent", activity,
        ...
        telementry.OnFinalize(request, response, "Club.Channel", "EnqueueEvent", activity,
