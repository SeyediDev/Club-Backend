...
از دایرکتوری `Club.Infrastructure` اجرا کنید:

```bash
cd D:\Projects\Club\Backend\src\Core\Club.Infrastructure

# ایجاد Migration
dotnet ef migrations add AddPlansTables --startup-project ..\..\CustomerPortal\Club.CustomerPortal.Api\Club.CustomerPortal.Api.csproj --context ClubContextCommand

# اعمال به دیتابیس
dotnet ef database update --startup-project ..\..\CustomerPortal\Club.CustomerPortal.Api\Club.CustomerPortal.Api.csproj --context ClubContextCommand
```
...
```bash
# حذف آخرین Migration
dotnet ef migrations remove --startup-project ..\..\CustomerPortal\Club.CustomerPortal.Api\Club.CustomerPortal.Api.csproj --context ClubContextCommand
```
...
```bash
# تولید SQL Script
dotnet ef migrations script --startup-project ..\..\CustomerPortal\Club.CustomerPortal.Api\Club.CustomerPortal.Api.csproj --context ClubContextCommand --output migration.sql
```
...
```bash
dotnet ef dbcontext list --startup-project ..\..\CustomerPortal\Club.CustomerPortal.Api\Club.CustomerPortal.Api.csproj
```

### خطای Build
اگر پروژه Build نمی‌شود:
```bash
cd D:\Projects\Club\Backend
dotnet build
```
