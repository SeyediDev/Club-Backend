# 🐳 راهنمای Docker و CI/CD برای Club Backend

این راهنما نحوه استفاده از فایل‌های Docker و CI/CD را برای پروژه Club Backend توضیح می‌دهد.
...
# Admin Panel
docker build -f Club/Backend/src/AdminPanel/Club.AdminPanel.Web/Dockerfile.new -t club-admin-web:latest .

# Channel API
docker build -f Club/Backend/src/Channel/Club.Channel.Api/Dockerfile.new -t club-channel-api:latest .

# Customer Portal API
docker build -f Club/Backend/src/CustomerPortal/Club.CustomerPortal.Api/Dockerfile.new -t club-customer-api:latest .
```
...
2. **فایل `.gitlab-ci.yml` را ویرایش کنید:**
   ```yaml
   variables:
     PROJECT_PATH: your-group/club-backend
...
environment:
  # Connection String
  - ConnectionStrings__DefaultConnection=Server=sqlserver;Database=ClubDb;...
